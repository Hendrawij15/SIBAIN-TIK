...
export const Login = async (req, res) => {
  ...
  if (!user) return res.status(404).json({ msg: "Akun tidak ditemukan" }); //jika email tidak ditemukan
  ...
  res.status(200).json({ id, nama, email, role }); //informasi login
};
...
export const STAFF = async (req, res) => {
  ...
  const user = await User.findOne({
    attributes: ["id", "nama", "email", "role"], //yang akan ditampilkan
    where: {
